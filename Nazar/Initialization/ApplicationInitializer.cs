...
public static class ApplicationInitializer
{
    ...
    {
        var button = EntityFactory.CreateButton(world, "Press Me!", new Pose(0.0f, 0, -0.5f, Quat.Identity));
        button.Set(new SubscriberComponent { EventName = "ButtonPressed" });

        var textWindow = EntityFactory.CreateTextWindow(world, "test", new Pose(-0.3f, 0, -0.5f, Quat.Identity));
        textWindow.Set(new SubscriberComponent { EventName = "ButtonPressed" });

        // Example Mesh entity
        EntityFactory.CreateMesh(world, Mesh.GenerateRoundedCube(Vec3.One * 0.1f, 0.02f), new Pose(0.3f, 0, -0.5f, Quat.Identity));

        // Example Model entity
        EntityFactory.CreateModelFromFile(world, "StereoKit.glb", new Pose(0.6f, 0, -0.5f, Quat.Identity));

        // Example Text entity
        EntityFactory.CreateText(world, "Example Text", new Pose(-0.9f, 0, -0.5f, Quat.Identity));
